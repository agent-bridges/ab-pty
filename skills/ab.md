...
that folder. Inside any existing AB shell, the wrapper asks the owning daemon
for a per-PTY `codex app-server` runtime and connects through its observed Unix
socket, so manually typed `codexs` gets authoritative `working`/`idle` status
just like an app-created Codex terminal. Runtime setup fails closed; there is
no heuristic fallback for this launch. The wrapper always adds
...
| **OpenAI Codex CLI** | `codexs` | `codex --dangerously-bypass-approvals-and-sandbox --remote <daemon-runtime> -C "$PWD" resume --last` | Use the wrapper. In any AB terminal it provisions the daemon-observed app-server runtime, resumes the latest session scoped to the current folder, sets the folder label, and always runs with full permissions. |
